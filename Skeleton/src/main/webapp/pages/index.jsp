...
<form:form method="post" modelAttribute="signupForm" action="create"
	...
	<fieldset>
		...
		<div
			class="control-group<c:if test="${not empty firstNameErrors}"> error</c:if>">
			<label class="control-label" for="field-firstName">First Name</label>
			<div class="controls">
				<form:input path="firstName" id="field-firstName" tabindex="2"
					maxlength="35" placeholder="First Name" />
		...
		<div
			class="control-group<c:if test="${not empty lastNameErrors}"> error</c:if>">
			<label class="control-label" for="field-lastName">Last Name</label>
			<div class="controls">
				<form:input path="lastName" id="field-lastName" tabindex="3"
					maxlength="35" placeholder="Last Name" />
				<form:errors path="lastName" cssClass="help-inline" element="span" />
			</div>
		</div>
		<label class="control-label" for="field-teamList">Team</label>
		<div class="controls">

			<form:select path="team">
				<c:forEach items="${teams}" var="teams">
					<form:option value="${teams.id}" label="${teams.teamName}" />
				</c:forEach>
			</form:select>
